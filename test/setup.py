#!/usr/bin/env python3
"""
Bootstrap projektu: tworzy komplet plikow (Modelfile, questions, script.py,
run) w katalogu PROJECT i nadaje prawa wykonywania.

Istniejacych plikow NIE nadpisuje - mozesz odpalic ponownie bez ryzyka.
...
# --- konfiguracja ---
PROJECT = "."          # "." = tutaj, albo np. "Template" / "test-2"
EXECUTABLE = ["script.py", "run"]
# --------------------

MODELFILE = r'''FROM gemma3:1b

SYSTEM """Jestes pomocnym asystentem. Odpowiadasz po polsku, zwiezle i konkretnie."""

PARAMETER temperature 0.3
PARAMETER num_ctx 4096
'''

QUESTIONS = r'''# Jedno pytanie = jedna linia. Linie z # sa pomijane.
Jakie sa zalety stali nierdzewnej w przemysle spozywczym?
Wyjasnij w trzech zdaniach, czym jest SEO.
'''

WRAPPER = r'''#!/usr/bin/env bash
# Wygodny skrot: ./run  zamiast  python3 script.py
cd "$(dirname "$0")" || exit 1
...
for name, content in FILES.items():
    ...
    if os.path.exists(path):
        print(f"  pomijam  {name}  (juz istnieje)")
        continue
...
print()
print("Gotowe. Dalej:")
print(f"  cd {PROJECT}" if PROJECT != "." else "  (jestes juz w katalogu projektu)")
print("  nano questions      # wpisz swoje pytania")
print("  nano Modelfile      # ustaw system prompt")
print("  ./run               # odpal caly przebieg")
